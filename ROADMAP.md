...
- **Security is asymptotic** (Shor–Preskill secure fraction): no finite-key bound yet, and the classical channel is unauthenticated. (Reconciliation via Cascade + privacy amplification via a Toeplitz hash now run on both distributed BB84 and E91 — Alice and Bob extract an identical secret key; the raw-socket `qne/bob.py` path is not yet wired.)
