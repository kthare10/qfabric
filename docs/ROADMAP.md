...
- ⬜ Quantify where **real classical-network effects** matter. The netem-vs-BB84-throughput study was **dropped 2026-09-25** with `notebooks/fabric/05_network_effects` and `run_network_conditions_experiment`: a deployed QKD link carries its classical channel on the same fiber route (~5 µs/km, no loss, negligible jitter), so impairing a shared WAN modelled a condition that does not occur, and the notebook's own QBER-per-condition panel was flat by construction. The realistic knob is the model-layer delay (`--channel-delay auto`), already carried by the global timeline. Where latency genuinely binds is herald delivery against memory coherence — see Phase 4's herald-latency dataset and Phase 6, where `apply_classical_netem` is retained for exactly that.
